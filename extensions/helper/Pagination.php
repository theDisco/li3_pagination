...
class Pagination extends \lithium\template\Helper {
	 ...
	 * @var array
	 */
	protected $_controls = array('first', 'previous', 'next', 'last');
	...
	/**
	 * Shorthand for first and previous controls.
	 *
	 * @return string
	 */
	public function pre() {
		return $this->controls('first') . $this->controls('previous');
	}

	/**
	 * Shorthand for next and last controls.
	 *
	 * @return string
	 */
	public function post() {
		return $this->controls('next') . $this->controls('last');
	}
	...
	public function pages(array $options = array()) {
		...
		$filter = function($self, $params) use ($router, $request, $extra) {
			...
			foreach ($pages as $_page) {
				...
				$page = $self->invokeMethod('_render', array(
					$extra['method'],
					'page',
					compact('url', 'label')
				));
				$_pages[] = $self->invokeMethod('_render', array(
					$extra['method'],
					'item',
					compact('page', 'options')
				));
			}
